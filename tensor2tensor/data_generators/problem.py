...
def preprocess_example_common(example, hparams, mode):
  ...
  if hparams.prepend_mode in ["prepend_inputs_masked_attention",
    ...
    if mode == tf.estimator.ModeKeys.PREDICT:
      batch_size = tf.shape(example["inputs"])[0]
      example["partial_targets"] = tf.concat(
          [example["inputs"], tf.zeros([batch_size, 1], tf.int64)], 1)
    else:
    ...
    if mode == tf.estimator.ModeKeys.PREDICT:
      batch_size = tf.shape(example["prepend"])[0]
      example["partial_targets"] = tf.concat(
          [example["prepend"], tf.zeros([batch_size, 1], tf.int64)], 1)
    else:
      example["targets"] = tf.concat(
